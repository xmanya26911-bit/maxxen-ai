...
export async function apiPost<T = any>(path: string, body: unknown, opts?: { timeoutMs?: number }): Promise<T> {
  ...
  const t = setTimeout(() => ctrl.abort(), opts && opts.timeoutMs ? opts.timeoutMs : 60000);
  try {
    ...
    if (e && e.name === "AbortError") throw new ApiError(408, "Request to " + path + " timed out.");
    if (e instanceof ApiError) throw e;
    throw new ApiError(0, (e && e.message) || "Network failed.");
  } finally {
    clearTimeout(t);
  }
}