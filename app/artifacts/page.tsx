          ...
          for (const c of chats) {
            ...
            for (const m of c.messages as any[]) {
              ...
              for (const b of blocks) {
                ...
                seen.add(key);
                found.push({ key, chatId: String(c.id || ""), title, lang: String(b.lang || "txt"), code: b.code, path: typeof b.path === "string" ? b.path : undefined, at });
              }
  ...
  return (
    <main className="min-h-screen text-white" style={{ background: "var(--mx-bg)" }}>
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <p style={{ color: "var(--mx-muted)", fontSize: 13, margin: "0 0 16px" }}>Every code build across your chats. Identical regenerations collapse into one.</p>
        {items.length === 0 ? (
          <div className="mx-panel">
            <h2>No artifacts yet</h2>
            <p className="sub">Ask Maxxen to build something in /chat — finished code collects here.</p>
            <div className="body">
              <div><a href="/chat" className="mx-btn" style={{ textDecoration: "none" }}>Build something</a></div>
            </div>
          </div>
        ) : (
              ...
              <div key={a.key} style={{ border: "1px solid var(--mx-border)", borderRadius: 12, overflow: "hidden", background: "var(--mx-surface)" }}>
                ...
                <div style={{ padding: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <button className="mx-btn-ghost" style={{ padding: "7px 12px", fontSize: 12 }} onClick={() => setOpen(open === a.key ? null : a.key)} aria-expanded={open === a.key} aria-label={(open === a.key ? "Close " : "View ") + (a.path || a.title)}>{open === a.key ? "Close" : "View"}</button>
                    {a.lang !== "html" && (
                      <button className="mx-btn-ghost" style={{ padding: "7px 12px", fontSize: 12 }} onClick={() => copy(a.code, a.key)} aria-live="polite">{copied === a.key ? "Copied ✓" : "Copy"}</button>
                    )}
