...
function toAccounts(data: any): Account[] {
  ...
  return list.map((a: any, i: number) => ({ id: String(a?.id ?? a?.accountId ?? ("row-" + i)), app: String(a?.toolkit ?? a?.appName ?? a?.app ?? a?.provider ?? "unknown"), status: String(a?.status ?? "connected"), created: String(a?.createdAt ?? a?.created_at ?? "") }));
}
      ...
      <div className="max-w-5xl mx-auto p-6">
          ...
          <div style={{ marginTop: 16, borderTop: "1px solid var(--mx-border)" }}>
            {accounts.map((a) => (
              <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 4px", borderBottom: "1px solid var(--mx-border)" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--mx-success)" }} />
                ...
                <button onClick={() => { try { localStorage.setItem("maxxen_agent_account", a.id); localStorage.setItem("maxxen_agent_app", a.app); } catch {} router.push("/chat"); }} style={{ background: "none", border: 0, color: "var(--mx-link)", fontSize: 12, cursor: "pointer", padding: 0 }} aria-label={"Use " + a.app + " in chat"}>Use in chat</button>
              </div>
            ))}
          </div>
        )}
        {accounts !== null && accounts.length === 0 && !loading && (
          <div className="mx-panel" style={{ marginTop: 16 }}>
            ...
            <p className="sub">Open app.composio.dev with YOUR key, connect Gmail, GitHub, Notion or Slack — then hit Refresh.</p>
          </div>
        )}
      </div>
    </main>
