...
const PRESETS: Record<string, { baseURL: string; model: string; keyHint: string }> = {
  ...
  useEffect(() => {
    // Background reconcile: if another tab/device pushed newer settings,
    // adopt them (at most once a minute — no polling hammer).
    // NOTE: intentionally after the early return — this effect only runs
    // once `ready` flips true, at which point hooks below are stable.
    if (!ready) return;
    const onFocus = async () => {
    ...
    <main className="min-h-screen text-white" style={{ background: "var(--mx-bg)" }}>
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <a href="/" className="font-black text-xl" style={{ textDecoration: "none", color: "inherit" }}>
            MAXXEN <span style={{ color: "var(--mx-link)" }}>AI</span>
          </a>
      ...
      <nav className="flex gap-2 px-6 py-3 border-b border-white/10 overflow-x-auto" aria-label="Settings sections">
        {(["endpoint", "plugins", "storage", "hosting"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} aria-pressed={tab === t} className="mx-tab" style={{ textTransform: "capitalize" }}>
            {t}
          </button>
        ))}
      </nav>
          ...
          <CPanel title="Your AI — ChatGPT, Claude, Gemini, or one key for all" sub="Tap a provider, paste the one key, done. All-in-one (OpenRouter) unlocks every model with a single key. Providers don't offer login-for-API, so one pasted key is the whole setup.">
            <div style={{ display: "flex", gap: 8 }}>
              {[
                ...
                <CButton key={p.id} variant={provider === p.id ? "primary" : "ghost"} onClick={() => pickPreset(p.id)}>
                  {p.label}
                </CButton>
              ))}
            </div>
            ...
            <p style={{ fontSize: 11, color: "var(--mx-meta)" }}>Key format: {PRESETS[provider]?.keyHint || "as issued by your provider"} · Base URL + Model ID prefilled above — edit freely for custom providers.</p>
            <CField placeholder="Model ID — e.g. gpt-4o-mini" value={model} onChange={(e) => setModel(e.target.value)} aria-label="Model ID" />
            <div>
              <CButton onClick={saveSettings}>Save endpoint</CButton>
            </div>
            <div>
              <CButton variant="danger" onClick={() => void forgetAll()}>
                Forget my vault
              </CButton>
            </div>
            <p style={{ fontSize: 11, color: "var(--mx-meta)", lineHeight: 1.6 }}>
              How this works: preferences sync as plain text, keys as AES-256-GCM ciphertext — both in YOUR private
              maxxen-data repo, per login email. Only this app server can unlock the vault. Wipe it any time here.
            </p>
          </CPanel>
        )}
        {tab === "plugins" && (
          <CPanel title="Your Composio — Your Plugins" sub="Paste YOUR Composio API key, then connect Gmail, GitHub, Notion, Slack, Vercel from YOUR dashboard. Manage them on /plugins.">
            ...
            <a href="/plugins" style={{ color: "var(--mx-link)", fontSize: 13 }}>
              Open the plugin hub →
            </a>
            <div>
              <CButton onClick={saveSettings}>Save</CButton>
            </div>
          </CPanel>
        )}
        {tab === "storage" && (
          <CPanel title="Your GitHub Storage (not mine)" sub="Paste YOUR GitHub token (repo scope). Maxxen uses maxxen-data in YOUR account. Browse it on /projects.">
            ...
            <div>
              <CButton variant="ghost" onClick={saveSettings}>
                Save
              </CButton>
            </div>
          </CPanel>
