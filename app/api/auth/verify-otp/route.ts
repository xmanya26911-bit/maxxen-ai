...
export async function POST(req: Request) {
  ...
  if (!key || !/^\S+@\S+\.\S+$/.test(key) || !clean || clean.length > 20) {
    return NextResponse.json({ error: "Valid email + code required." }, { status: 400 });
  }
  ...
  // Stateless path: works on any serverless instance.
  // Rate-limited (6 tries/ticket) + single-use (replay blocked). Generic errors only — no oracle.
  if (ticket) {
    const t = String(ticket);
    if (isTicketUsed(t)) {
      return NextResponse.json({ error: "Code already used — request a new one." }, { status: 401 });
    }
    if (!checkTicketRateLimit(key, t)) {
  ...
  purgeExpired(); // drop anything older than 10 min
