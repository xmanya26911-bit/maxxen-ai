...
export async function POST(req: Request) {
  try {
    ...
    if (!vercelToken || typeof vercelToken !== "string") return NextResponse.json({ error: "vercelToken required — YOUR token (Settings → Hosting)." }, { status: 400 });
    if (!projectName || typeof projectName !== "string") return NextResponse.json({ error: "projectName required." }, { status: 400 });
    if (!Array.isArray(files) || !files.length) return NextResponse.json({ error: "files[] required: [{ file: 'index.html', data: '…' }]." }, { status: 400 });
    ...
    const r = await fetch("https://api.vercel.com/v13/deployments", {
      ...
      body: JSON.stringify({ name: projectName, target: target === "preview" ? "preview" : "production", files: clean, projectSettings: { framework: null } }),
    });
    ...
    return NextResponse.json({ ok: true, id: (j as any).id, url: (j as any).url, inspectorUrl: "https://vercel.com/dashboard", status: (j as any).readyState || (j as any).status || "QUEUED" });
  } catch (e: any) {
