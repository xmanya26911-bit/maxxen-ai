...
export async function POST(req: Request) {
  try {
    ...
    const body: StoredSettings = wipe ? { updatedAt: new Date().toISOString(), prefs: {}, vault: null } : { updatedAt: new Date().toISOString(), prefs: { ...(prev && typeof prev.prefs === "object" ? prev.prefs : {}), ...cleanPrefs }, vault: Object.keys(cleanSecrets).length ? sealSecrets(email, cleanSecrets) : null };
    await oct.rest.repos.createOrUpdateFileContents({ owner: me.login, repo, path: SETTINGS_PATH, message: "maxxen: sync preferences + encrypted vault", content: Buffer.from(JSON.stringify(body, null, 2)).toString("base64"), sha });
    return NextResponse.json({ ok: true, repo: me.login + "/" + repo, savedSecrets: !wipe && Object.keys(cleanSecrets).length > 0, wiped: !!wipe });
  } catch (e: any) {
