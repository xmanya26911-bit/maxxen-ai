...
export async function POST(req: Request) {
  try {
    ...
    if (safePath === null) return NextResponse.json({ error: "path must be root, builds/, chats/, or settings.json." }, { status: 400 });
    ...
    try {
      ...
      if (e?.status === 404) return NextResponse.json({ ok: true, repo: me.login + "/" + repo, path: safePath, entries: [], empty: true });
      throw e;
    }
    ...
    if (!Array.isArray(cur.data)) {
      ...
      return NextResponse.json({ ok: true, repo: me.login + "/" + repo, path: safePath, file: { name: d.name, path: d.path, type: d.type, size: d.size ?? 0, text } });
    }
    const items = cur.data.map((e: any) => ({ name: e.name, path: e.path, type: e.type, size: e.size ?? 0 }));
