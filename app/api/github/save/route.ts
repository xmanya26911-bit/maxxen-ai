...
export async function POST(req: Request) {
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        await oct.rest.repos.createOrUpdateFileContents({ owner: me.login, repo, path: safePath, message: typeof message === "string" && message ? message.slice(0, 140) : ("maxxen: save " + safePath), content: encoded(), sha });
