...
export async function POST(req: Request) {
  try {
    ...
    const commits = await oct.rest.repos.listCommits({ owner: me.login, repo: "maxxen-data", path: safePath, per_page: Math.min(Math.max(Number(limit) || 10, 1), 30) });
    return NextResponse.json({ ok: true, history: commits.data.map((c: any) => ({ sha: c.sha, message: c.commit?.message?.split("\n")[0] || "", date: c.commit?.author?.date || null, author: c.commit?.author?.name || "" })) });
  } catch (e: any) {
