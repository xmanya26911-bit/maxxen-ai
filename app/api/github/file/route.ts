...
export async function POST(req: Request) {
  try {
    ...
    if (sha !== undefined && (typeof sha !== "string" || !sha.trim() || sha.length > 100)) return NextResponse.json({ error: "Invalid sha." }, { status: 400 });
    ...
    if (typeof sha !== "string" || !sha.trim()) return NextResponse.json({ error: "sha required for rollback." }, { status: 400 });
    ...
    await oct.rest.repos.createOrUpdateFileContents({ owner: me.login, repo, path: safePath, message: "maxxen: rollback " + safePath + " to " + sha.trim().slice(0, 7), content: Buffer.from(old.text).toString("base64"), sha: curSha });
