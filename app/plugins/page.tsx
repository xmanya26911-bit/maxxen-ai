...
export default function Plugins() {
  ...
  async function check(slug?: string) {
    ...
    const tk = (slug ?? toolkit).toLowerCase().trim();
    if (!useKey) {
      setStatus("Paste YOUR Composio API key first — it never leaves your browser.");
      return;
    }
    if (!tk) {
      setStatus("Type a toolkit slug — e.g. gmail.");
      return;
    }
    if (checking) return;
    setChecking(true);
    setToolkit(tk);
    setStatus("Checking " + tk + " on YOUR account…");
    try {
      const r = await fetch("/api/composio/connect", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ composioKey: useKey, toolkit: tk }) });
      const j = await r.json();
      if (j.error) setStatus(j.error);
      else {
        ls("maxxen_composio_key", useKey);
        const hint = typeof j.hint === "string" && j.hint ? j.hint : ("Checked " + tk + " on YOUR account.");
        setStatus(hint + " To add it, connect " + tk + " at app.composio.dev, then check again.");
      }
    } catch (e: any) {
    ...
    <main className="min-h-screen text-white" style={{ background: "var(--mx-bg)" }}>
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="mx-panel" style={{ marginTop: 12 }}>
          ...
          <p className="sub">Type any toolkit slug, or tap one below to check it instantly.</p>
          <div className="body">
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {POPULAR.map((t) => (
                <button key={t} className="mx-btn-ghost" style={{ padding: "7px 12px", fontSize: 12 }} onClick={() => void check(t)} aria-pressed={toolkit === t} title={"Check " + t}>{t}</button>
              ))}
            </div>
